...
export default function HeroBento({
  ...
  return (
    <section className="bento-grid mb-16">
      ...
      <div className="col-span-12 lg:col-span-8 row-span-2 bg-white rounded-[32px] p-8 md:p-12 relative overflow-hidden flex flex-col justify-center shadow-sm hover-lift group">
        <div className="relative z-10 max-w-lg">
          <span className="inline-block px-3 py-1 bg-orange-100 text-accent text-xs font-bold tracking-widest uppercase rounded-full mb-6">
            Flagship Collection
          </span>
          <h1 className="text-5xl md:text-6xl font-black leading-[0.95] mb-6 tracking-tight">
            {heroProduct.brand}
            <br />
            <span className="text-accent">{heroProduct.name.replace(heroProduct.brand, "").trim() || heroProduct.name}</span>
          </h1>
          <p className="text-lg text-secondary mb-4 leading-relaxed max-w-sm">
          ...
          <div className="flex items-center gap-4">
            <Link
              ...
              className="bg-main text-white h-12 px-8 rounded-full font-bold text-sm tracking-wide hover:bg-accent hover:shadow-lg hover:shadow-orange-500/20 transition-all flex items-center gap-2"
            >
              SHOP NOW
              <ArrowRightIcon />
            </Link>
            <Link
              href="/search"
              className="text-main font-bold text-sm border-b-2 border-transparent hover:border-main transition-all"
            >
              View All Products
            </Link>
          </div>
      ...
      <Link
        ...
        className="col-span-12 md:col-span-6 lg:col-span-4 bg-main rounded-[32px] p-8 text-white relative overflow-hidden flex flex-col justify-between min-h-[280px] hover-lift group"
      >
        ...
        {/* Real product image */}
        <div className="absolute -right-2 bottom-0 w-40 h-40 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-[-5deg]">
          <Image
      ...
      {/* New Arrivals card */}
      <div className="col-span-12 md:col-span-6 lg:col-span-4 bg-white border border-main/5 rounded-[32px] p-8 relative overflow-hidden flex flex-col justify-between min-h-[200px] hover-lift group">
        <div className="relative z-10">
